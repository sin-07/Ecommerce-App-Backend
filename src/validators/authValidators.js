...
export const validateOtpVerification = (req) => {
  ...
  if (!/^\d{6}$/.test(String(req.body.otp || '').trim())) errors.push('A 6-digit OTP is required');
  return errors;
};
...
export const validateRegister = (req) => {
  ...
  if (!name || String(name).trim().length < 2) errors.push('Full name must be at least 2 characters');
  ...
  if (!otp || !/^\d{6}$/.test(String(otp).trim())) errors.push('A valid 6-digit verification code is required');
